shinyServer(function(input, output) {
  ...
  output$plot1_a <- renderPlotly({
    ...
    # Plot 2 Lolipop: Top 10 Gaming Channel--------------------------------------------------------------------------------------------------
    output$plot2_a <- renderPlotly({   
    # Creating Data
    #Goal Data
    ...
    index_long <- index[, -c(1,4)] %>%
      group_by(year) %>% 
      summarise(avg_score = mean(input$input_score)) %>%
      filter(avg_sc)
      ungroup() %>% 
      mutate(
  ...
  # Plot 3 Lolipop: Viewers Activity of Gaming Videos--------------------------------------------------------------------------------------------------  
  output$plot3_a <- renderPlotly({
    ...
    plot3 <- ggplot(data = report_1, aes(x = value,
      ...
      geom_col(width = 0.5) +
      labs(title = glue("Top 10  Countries Located in {input$input_region} according to SDG Score "),
           x = "Countries",
           y = NULL) +
